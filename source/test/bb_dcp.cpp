...
TEST_F(Ddcp, ident_cnf_invalid) {
  ...
  dcp.mcs_ctx.external_interface_id = 0x1000;
  dcp.mcs_ctx.xid = 0xBEEF;

  /* drop first 16 bytes */
  frame->erase(frame->begin(), frame->begin() + 16);
  EXPECT_EQ(dcp_srv_ident_cnf(&dcp, &dcp.mcs_ctx, frame->data(), frame->size(), NULL), -SPN_ENXIO);
}
...
TEST_F(Ddcp, ident_cnf_ecopn) {
  ...
  /* drop first 16 bytes */
  frame->erase(frame->begin(), frame->begin() + 16);
  EXPECT_EQ(dcp_srv_ident_cnf(&dcp, &dcp.mcs_ctx, frame->data(), frame->size(), NULL), SPN_OK);
